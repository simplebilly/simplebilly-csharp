/*
 * SimpleBilly API
 *
 * Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: support@simplebilly.com
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using FileParameter = Org.OpenAPITools.Client.FileParameter;
using OpenAPIDateConverter = Org.OpenAPITools.Client.OpenAPIDateConverter;

namespace Org.OpenAPITools.Model
{
    /// <summary>
    /// LegalDocument
    /// </summary>
    [DataContract(Name = "LegalDocument")]
    public partial class LegalDocument : IValidatableObject
    {

        /// <summary>
        /// Gets or Sets DocType
        /// </summary>
        [DataMember(Name = "docType", IsRequired = true, EmitDefaultValue = true)]
        public LegalDocType DocType { get; set; }

        /// <summary>
        /// Gets or Sets Lang
        /// </summary>
        [DataMember(Name = "lang", IsRequired = true, EmitDefaultValue = true)]
        public LanguageCode Lang { get; set; }
        /// <summary>
        /// Initializes a new instance of the <see cref="LegalDocument" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected LegalDocument() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="LegalDocument" /> class.
        /// </summary>
        /// <param name="content">Plain text, &#x60;\\n\\n&#x60; separates paragraphs. (required).</param>
        /// <param name="docType">docType (required).</param>
        /// <param name="lang">lang (required).</param>
        /// <param name="title">title (required).</param>
        public LegalDocument(string content = default, LegalDocType docType = default, LanguageCode lang = default, string title = default)
        {
            // to ensure "content" is required (not null)
            if (content == null)
            {
                throw new ArgumentNullException("content is a required property for LegalDocument and cannot be null");
            }
            this.Content = content;
            this.DocType = docType;
            this.Lang = lang;
            // to ensure "title" is required (not null)
            if (title == null)
            {
                throw new ArgumentNullException("title is a required property for LegalDocument and cannot be null");
            }
            this.Title = title;
        }

        /// <summary>
        /// Plain text, &#x60;\\n\\n&#x60; separates paragraphs.
        /// </summary>
        /// <value>Plain text, &#x60;\\n\\n&#x60; separates paragraphs.</value>
        [DataMember(Name = "content", IsRequired = true, EmitDefaultValue = true)]
        public string Content { get; set; }

        /// <summary>
        /// Gets or Sets Title
        /// </summary>
        [DataMember(Name = "title", IsRequired = true, EmitDefaultValue = true)]
        public string Title { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class LegalDocument {\n");
            sb.Append("  Content: ").Append(Content).Append("\n");
            sb.Append("  DocType: ").Append(DocType).Append("\n");
            sb.Append("  Lang: ").Append(Lang).Append("\n");
            sb.Append("  Title: ").Append(Title).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
